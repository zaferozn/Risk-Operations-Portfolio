# Risk Operations Portfolio

This repository documents my transition into risk, financial crime, due diligence, fraud-risk, and security operations through case-based notes and structured reporting practice.

## Focus Areas

- Financial crime fundamentals
- AML / KYC concepts
- Fraud-risk patterns
- Account takeover risk
- Due diligence research
- Adverse media review
- Sanctions screening
- Case documentation
- Escalation-quality reporting

## Background

My background includes correctional security operations, operational record tracking, formal incident documentation, surveillance monitoring, archival evidence analysis, and doctoral-level research on crime, public order, and institutional security.

I am building this portfolio to demonstrate how my experience in evidence review, procedural documentation, monitoring, and structured reporting can transfer into risk, financial crime, due diligence, and security operations.

## Training

- Operational Analysis of Suspicious Transaction Reports — Basel Institute on Governance / International Centre for Asset Recovery, 2026
  - Completed introductory training on STR review, hypothesis building, information collection, FIU database review logic, likelihood and consequence assessment, and evidence-based reporting.

## Repository Structure

concepts/
simulated-cases/
templates/
glossary/

## Current Goal

To build a practical portfolio that demonstrates:

- evidence review
- risk assessment
- suspicious activity analysis
- structured documentation
- escalation discipline
- clear professional reporting in English

## Note

All cases in this repository are simulated for learning and portfolio purposes. They do not contain real customer data, real institutional information, or confidential operational details.
